...
test('认不出国别的节点分到「其他地区」组,不和兜底站点集「其他」撞出站 tag', () => {
  const { groups } = groupNodesByRegion([{ tag: '其他-01' }, { tag: '破晓 | 其他-02' }, { tag: '香港-01' }])
  assert.deepEqual(groups.map((g) => [g.name, g.nodeTags]), [
    ['其他地区', ['其他-01', '破晓 | 其他-02']],
    ['香港', ['香港-01']],
  ])
})
